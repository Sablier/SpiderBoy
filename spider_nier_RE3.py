...
def find_imgs(url):
    ...
    img_addr_list = re.findall(pattern,html)
    print(img_addr_list)
...
def save_imgs(path,name,img_addr_list2):
    ...
    for each in img_addr_list2:
        picture = url_open(each[1])
        print("saving picture %s" % each[1])

        with open( path + str(name) + "_" + each[0] + ".jpg" ,"wb") as file:
            file.write(picture)
        print()
...
def main():
    ...
    path = "C:/Users/BiaobiaoPeng/Desktop/spiderboy/imgs-neir-RE3/"
